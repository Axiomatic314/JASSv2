...
namespace JASS
	...
	*/
	...
	class query_heap_counter : public query
		{
		private:
		...
		public:
			/*
				QUERY_HEAP_COUNTER::QUERY_HEAP_COUNTER()
				------------------------
			*/
			/*!
				@brief Constructor
			*/
			query_heap_counter(compress_integer &codex) :
				query(codex),
				zero(0),
			...
			*/
			/*!
				@brief Destructor
			*/
			virtual ~query_heap_counter()
				{
				/* Nothing */
				}

			/*
				QUERY_HEAP_COUNTER::INIT()
				------------------
			*/
			/*!
...
//					std::partial_sort(accumulator_pointers + needed_for_top_k, accumulator_pointers + top_k, accumulator_pointers + top_k)
			...
			*/
			forceinline void add_rsv(DOCID_TYPE document_id, ACCUMULATOR_TYPE score)
				...
				*which.pointer() += score;
				...
				ACCUMULATOR_TYPE which_value = accumulators.get_value(document_id);
				/*
					accumulator is less than the heap entry value
				*/
				if (which_value < top_k_lower_bound)
					return;
				/*
				...
				*/
				if (needed_for_top_k > 0)
					...
					*/
					if (which_value - score < top_k_lower_bound)
						{
			...
			/*
				QUERY_HEAP_COUNTER::DECODE_WITH_WRITER()
				--------------------------------
			*/
			/*!
			...
			*/
			static void unittest(void)
				...
				query_heap_counter *query_object = new query_heap_counter(codex);
				...
				*/
				...
				puts("query_heap_counter::PASSED");
				}
		};
	}
