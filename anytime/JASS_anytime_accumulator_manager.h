	...
	*/
	JASS::query *get_by_name(const std::string &name, JASS::compress_integer &codex)
		...
		if (name == "2d_heap")
			...
			return new JASS::query_heap_counter<JASS::accumulator_counter_prefix<JASS::query::ACCUMULATOR_TYPE, JASS::query::MAX_DOCUMENTS>>(codex);
		else
			{
