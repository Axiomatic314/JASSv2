...
enum metric_index
	...
	BUYING_POWER_NORMALIZED_DISCOUNTED_CUMULATIVE_GAIN,		///< buying power normalized discounted cumulative gain (if we are an eCommerce metric).
...
const std::vector<std::string> metric_name =
	...
	"Buying Power Normalized Discounted Cumulative Gain (BPnDCG)",
	"SENTINAL"
	};
	...
	public:
		...
		*/
		/*!
			...
			@param buying_power_normalized_discounted_cumulative_gain [in] The buying power nDCG of this query.
		*/
		metric_set(const std::string &run_id, const std::string &query_id, size_t relevant_count, size_t returned, size_t relevant_returned, double mean_reciprocal_rank, double mean_reciprocal_rank_4k, double precision, double recall, double expected_search_length, double f1,
			...
			double price_based_normalized_discounted_cumulative_gain, double buying_power_normalized_discounted_cumulative_gain) :
			run_id(run_id),
			query_id(query_id)
			{
			...
			metric[BUYING_POWER_NORMALIZED_DISCOUNTED_CUMULATIVE_GAIN] = buying_power_normalized_discounted_cumulative_gain;
			}
...
metric_set evaluate_query(const std::string &run_id, const std::string &query_id, std::vector<std::string> &results_list, std::shared_ptr<JASS::evaluate> gold_standard_price, std::shared_ptr<JASS::evaluate>gold_standard_assessments, size_t depth)
	...
	double buying_power_normalized_discounted_cumulative_gain = 0;
	...
	if (gold_standard_price->assessments.size() != 0)
		...
		JASS::evaluate_buying_power_normalized_discounted_cumulative_gain buying_power_normalized_discounted_cumulative_gain_computer(gold_standard_price, gold_standard_assessments);
		buying_power_normalized_discounted_cumulative_gain = buying_power_normalized_discounted_cumulative_gain_computer.compute(query_id, results_list, depth);
		}
	...
	return metric_set(run_id, query_id, number_of_relvant_assessments, returned, relevant_returned, mrr, mrr4k, precision, recall, esl, f1,
		...
		price_based_normalized_discounted_cumulative_gain, buying_power_normalized_discounted_cumulative_gain);
	}
